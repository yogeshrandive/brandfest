#!/usr/bin/env node
/**
 * Downloads Poppins font files required by the poster renderer.
 * Runs automatically via `postinstall` so Vercel picks them up at build time.
 */
...
async function main() {
  ...
  for (const font of FONTS) {
    ...
    if (fs.existsSync(dest)) {
      console.log(`  ✓ ${font.name} already present`);
      continue;
    }
